<template>
	<!--game推荐组件-->
	<div v-if="data.length > 0" class="recommend-content" id="recommend-content">
		...
		<div class="recommend-rows-wrap">
			<waterfall :col="col" :data="data" :is-transition="true" :lazyDistance="0" @loadmore="loadmore">
				<template>
					<div class="recommend-row-block">
						<div class="recommend-row" v-for="(item, index) in data" :key="index" @click="onRecommendClick(item, index)">
						...
						<div v-show="page >= totalPage" class="recommend-bottom-tip">已经到底了噢~</div>
						<div v-show="loading" class="recommend-bottom-loading">
							<span class="recommend-bottom-loading-inner"></span>
							<span class="recommend-bottom-loading-text">正在加载中...</span>
						</div>
					</div>
				</template>
...
export default {
	...
	computed: {
		...
		page() {
			return this.$store.state.recommendConfig.page;
		},
		totalPage() {
			return this.$store.state.recommendConfig.total_page;
		},
		data() {
			return this.$store.state.recommendConfig.recommendArray;
		}
	},
	methods: {
		...
		loadmore() {
			console.log("===========recommend.loadmore==========")
			if (this.isRequesting || this.page >= this.totalPage) return false;
			this.loading = true;
			setTimeout(() => {
				this.isRequesting = true;
				ConfigService.getRecommendConfig({
					channel_id: this.channelId,
					app_id: this.appId,
					page: this.page + 1,
					callback: (res, err) => {
						if (err || res.code !== 0) return this.$toast(err);
						this.$store.commit("updateRecommendConfig", res.body);
						this.loading = false;
...
.recommend-content {
	...
	.recommend-rows-wrap {
		...
		.recommend-bottom-loading {
			...
			.recommend-bottom-loading-inner {
				display: block;
				position: relative;
				width: 18px;
				height: 18px;
				...
				border-radius: 50%;
				box-sizing: border-box;
				border: 1px solid #f3730b;
				border-right-color: transparent;
				animation: spin 3s linear infinite;
			}

			.recommend-bottom-loading-inner:before {
				content: "";
				position: absolute;
				top: 2px;
				left: 2px;
			...
			.recommend-bottom-loading-inner:after {
				content: "";
				position: absolute;
				top: -4px;
				left: -4px;
				...
				bottom: -4px;
				border-radius: 50%;
				border: 1px solid #ff1edd;
				border-bottom-color: transparent;
				animation: spin 1.5s linear infinite;
			}

			.recommend-bottom-loading-text {
				display: block;
				height: 20px;
				font-size: 14px;
				line-height: 20px;
